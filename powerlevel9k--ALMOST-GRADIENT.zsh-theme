...
prompt_dir() {
  ...
  if [[ -n "$POWERLEVEL9K_SHORTEN_DIR_LENGTH" ]]; then
    ...
    case "$POWERLEVEL9K_SHORTEN_STRATEGY" in
   	 ...
   	 # START -- ICONIFY STRATEGIES - Written by Jeremy Sarda
      ...
      iconify_then_truncate_from_right)
		  ...
		  stripped_pwd="%f%k%F{white}%K{black}"$PWD # Define root PWD with the computer icon at the beginning
		  
		  
		  # Replace Home with "~" automatically before executing icon rules.
		  # ICONIFIED_PWD = $(pwd | sed -e "s,^$HOME,~,")
		  
          #$'/Users/mac/Projects'         				$'\UE832'				#  # Open Book with -> Arrow
		  ...
		  # TODO: Replace the most specific paths first. Sort array by length of key.
		  #
		  # for ((i=1; i <= $#values; i--)) { combined[i]=($values[i]$'\0'$keys[i]); }
		  # iconify_prefix_rules=${(Oa)iconify_prefix_rules}
		  
		  # Split keys and values into separate arrays. Don't think I need this any more.
		  iconify_rule_keys=("${(@k)iconify_rules}")
		  ...
		  # Only the most specific matched key will show the directory name and icon.
		  # All matches after the first will only show the icon. I'm keeping a running count
		  ...
		  for ((i=0; i <= $pwd_size; i++)) {
			...
			if (( ${+iconify_rules[`print $pwd_portion_to_check`]} )); then
				...
				dir_label="${iconify_rules[`print $pwd_portion_to_check`]} $dir_name "
				...
				local iconified_segment="$before_segment $dir_label $path_separator%F{white} "
				...
				# DEBUG
				echo "*Matched* Dir #$i:\t\t[ $stripped_pwd %k%f ]\t\t\t\t$pwd_portion_to_check"
				...
				echo "!matched  Dir #$i:\t\t[ $dir_label ]\t\t\t\t$pwd_portion_to_check"
				#continue
				
			fi

			echo "SEGMENT_COLOR: $SEGMENT_COLOR"
		  ...
		  current_path="%f%F{white}$stripped_pwd"
